...
var _stringifyObject2 = require('stringify-object');

var _stringifyObject3 = _interopRequireDefault(_stringifyObject2);
...
var NEGATION_FLAG = 'enzyme-assertion-is-negated';
 ...
 */
var isNegated = function isNegated(ctx) {
  return Boolean(ctx[NEGATION_FLAG]);
};
 ...
 * @return {undefined}
 */
var assert = function assert(_ref) {
  var statement = _ref.statement,
      _ref$ctx = _ref.ctx,
      ctx = _ref$ctx === undefined ? {} : _ref$ctx,
      expected = _ref.expected,
      actual = _ref.actual,
      msg = _ref.msg;

  // Detect negated assertions.
  var negated = isNegated(ctx);
  var correct = Boolean(negated ? !statement : statement);
  ...
  var details = msg instanceof Function ? msg(not) : msg;
 ...
 */
var negate = function negate(methodName) {
  return function () {
    this[NEGATION_FLAG] = true;
    var assertion = this[methodName];
 ...
 */
var isEnzymeWrapper = function isEnzymeWrapper(actual) {
  if (!actual) {
    return false;
  }

  // Duck typed (instanceof can break if duplicate package instances exist).
  return Boolean(typeof actual.setState === 'function' && typeof actual.findWhere === 'function' && typeof actual.debug === 'function');
};
 ...
 */
var assertIsEnzymeWrapper = function assertIsEnzymeWrapper(actual) {
  return assert({
 ...
 */
var stringifySelector = function stringifySelector(selector) {
  ...
  if (type === 'function') {
    return (0, _reactDisplayName2.default)(selector);
  }

  // Nope, enzyme attribute selector.
  return (0, _stringifyObject3.default)(selector, {
    // Prevents newlines.
 ...
 */
var addEnzymeSupport = function addEnzymeSupport(defaultAssertion, enzymeAssertion) {
...
exports.default = function (original) {
  return {
    ...
    toHaveProp: addEnzymeSupport(original.toHaveProp, function (prop, value) {
      ...
      var actual = this.actual.props();
      var displayName = this.actual.name();
      var real = _defineProperty({}, prop, actual[prop]);
      var expected = _defineProperty({}, prop, value);
      ...
      if (!isNegated(this) || value === undefined) {
        assert({
          ...
          msg: function msg(not) {
            return 'Expected ' + displayName + ' to ' + not + 'have prop "' + prop + '"';
          }
        });
      }
      ...
      if (value !== undefined) {
        assert({
          ctx: this,
          statement: (0, _deepEql2.default)(actual[prop], value),
          expected: expected,
          actual: real,
          msg: function msg(not) {
            var constructor = (value.constructor || {}).name || 'Object';
            var prettyValue = value instanceof Object ? constructor + ' {...}' : value;
     ...
     * @return {this} - The expectation context.
     */
    toNotHaveProps: addEnzymeSupport(original.toNotHaveProps, negate('toHaveProps')),
    ...
    toHaveClass: addEnzymeSupport(original.toHaveClass, function (className) {
      ...
      var actual = element.prop('className') ? element.prop('className').split(' ') : [];

      var expected = String(className).split(' ');

      if (isNegated(this)) {
        // Reimagine `actual` without the classname.
        expected = actual.filter(function (value) {
          return value !== className;
        });
      } else {
        var _expected2;

        (_expected2 = expected).push.apply(_expected2, _toConsumableArray(actual));
      }
      ...
      assert({
        ...
        actual: actual,
        msg: function msg(not) {
          return 'Expected ' + element.name() + ' to ' + not + 'have class "' + className + '"';
        }
      });
     ...
     * @return {this} - The expectation context.
     */
    toNotHaveClass: addEnzymeSupport(original.toNotHaveClass, negate('toHaveClass')),
    ...
    toHaveState: addEnzymeSupport(original.toHaveState, function (expectedState) {
      ...
      assert({
        ...
        msg: 'expect(...).toHaveState expects an object,' + (' was given "' + expectedState + '"')
      });

      // Check every property in the expected state.
      Object.keys(expectedState).forEach(function (key) {
        var actual = element.state(key);
        var expected = expectedState[key];

        // Deeply check equivalence.
        assert({
          ctx: _this2,
          statement: (0, _deepEql2.default)(actual, expected),
          expected: expected,
          actual: actual,
          msg: function msg(not) {
            return 'Expected state "' + key + '" to ' + not + 'equal ' + expected;
          }
        });
      });
     ...
     * @return {this} - The expectation context.
     */
    toNotHaveState: addEnzymeSupport(original.toNotHaveState, negate('toHaveState')),
    ...
    toHaveRendered: addEnzymeSupport(original.toHaveRendered, function (element) {
      ...
      var exists = Boolean(actual && actual.length);
      var testEquality = element !== undefined;
      var negated = isNegated(this);
      ...
      if (!negated) {
        var unexpectedNull = !exists || testEquality ? false : actual.equals(null);
      ...
      assert({
        ...
        msg: function msg(not) {
          var isValidElement = _react2.default.isValidElement(element);
          var hasProps = isValidElement && Object.keys(element.props).length;

          // Show abbreviated element on one line.
          var expectedString = isValidElement ? (0, _elementToString2.default)(element) : '"' + element + '"';

          var indent = hasProps ? '\n\t' : '';
          var colon = indent ? ':' : ' ';

          return 'Expected element to ' + not + 'equal' + ('' + colon + indent + expectedString);
        }
      });
    }),
     ...
     * @return {this} - The expectation context.
     */
    toNotHaveRendered: addEnzymeSupport(original.toNotHaveRendered, negate('toHaveRendered')),
     ...
     * @return {this} - The expectation context.
     */
    toHaveStyle: addEnzymeSupport(original.toHaveStyle, function (property, value) {
      var _this3 = this;
      ...
      var styles = property instanceof Object ? property : _defineProperty({}, property, value);
      ...
      Object.keys(styles).forEach(function (property) {
        ...
        if (value === undefined) {
          ...
          assert({
            ...
            msg: function msg(not) {
              return 'Expected ' + displayName + ' to ' + not + 'have css property "' + property + '"';
            }
          });
        } else {
          // Show what css is expected.
          var styleString = (0, _stringifyObject3.default)(_defineProperty({}, property, value), {
            inlineCharacterLimit: Infinity
          });
          ...
          assert({
            ...
            actual: style,
            msg: function msg(not) {
              return 'Expected ' + displayName + ' to ' + not + 'have css ' + styleString;
            }
          });
        }
      });
     ...
     * @return {this} - The expectation context.
     */
    toNotHaveStyle: addEnzymeSupport(original.toNotHaveStyle, negate('toHaveStyle')),
     ...
     * @return {this} - The expectation context.
     */
    toHaveContext: addEnzymeSupport(original.toHaveContext, function toHaveContext(context) {
      var _this4 = this;

      var element = this.actual;
      assertIsEnzymeWrapper(element);

      var actual = element.context();

      Object.keys(context).forEach(function (property) {
        var expected = context[property];
        var expectedString = (0, _stringifyObject3.default)(expected, {
          inlineCharacterLimit: Infinity
        });

        assert({
          ctx: _this4,
          statement: (0, _deepEql2.default)(actual[property], expected),
          actual: actual[property],
          expected: expected,
          msg: function msg(not) {
            return 'Expected context property' + (' "' + property + '" to ' + not + 'equal ' + expectedString);
          }
        });
      });
     ...
     * @return {this} - The expectation context.
     */
    toNotHaveContext: addEnzymeSupport(original.toNotHaveContext, negate('toHaveContext')),
    ...
    toBeA: addEnzymeSupport(original.toBeA, function (type) {
      ...
      var displayName = (0, _reactDisplayName2.default)(type);
      var element = this.actual;
      var _article = this.article,
          article = _article === undefined ? 'a' : _article;
      ...
      assert({
        ...
        actual: name,
        msg: function msg(not) {
          return 'Expected ' + element.name() + ' to ' + not + 'be ' + article + ' ' + displayName;
        }
      });
    }),
    ...
    toExist: addEnzymeSupport(original.toExist, function () {
      assert({
        ...
        msg: function msg(not) {
          return 'Expected element to ' + not + 'exist';
        }
      });
    }),
    ...
    toContain: addEnzymeSupport(original.toContain, function (selector) {
      ...
      var isContained = element.find(selector).length > 0;
      var stringSelector = stringifySelector(selector);

      assert({
        ctx: this,
        statement: isContained,
        msg: function msg(not) {
          return 'Expected element to ' + not + 'contain "' + stringSelector + '"';
        }
      });
    }),
